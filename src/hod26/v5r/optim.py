"""Optimizer families for the repaired V5R localization lineage."""
...
def _is_fast_hsi_parameter(name: str) -> bool:
...
def _is_localization_parameter(name: str) -> bool:
    return name.startswith(
        (
...
class HOD26V5RLayerDecayOptimizerConstructor(HOD26LayerDecayOptimizerConstructor):
    """Keep pretrained localization at detector LR instead of V5's unsafe 8x."""
    ...
    def add_params(self, params, module, **kwargs) -> None:
        ...
        localization_lr_mult = float(
            self.paramwise_cfg.get("localization_lr_mult", 1.0)
        )
        class_lr_mult = float(self.paramwise_cfg.get("class_lr_mult", 4.0))
        if localization_lr_mult <= 0:
            raise ValueError("localization_lr_mult must be positive")
        grouped = defaultdict(lambda: {"params": [], "param_names": []})
        for name, parameter in module.named_parameters():
            if not parameter.requires_grad:
                continue
            if _is_localization_parameter(name):
                scale, family = localization_lr_mult, "localization"
            elif _is_fast_hsi_parameter(name):
                scale, family = hsi_lr_mult, "hsi"
